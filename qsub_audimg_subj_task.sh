...
do
    for task in pch-class pch-height pch-hilo timbre pch-helix-stim-enc pch-classX timbreX; 
    do
	# Do not overwrite results, remove them if recompute required
	if [ ! -e results_audimg_subj_task_mkc_N100/${subj}_${task}_res_part.pickle ];
	then
	    echo ${subj} ${task}
